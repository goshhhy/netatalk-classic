		   ...
		   const int tickleval)
{
	...
	if (!asp->inited) {
		...
		if (!asp_ac && (asp_ac = (struct asp_child **)
				calloc(server_children->nsessions,
				       sizeof(struct asp_child *)))
		    == NULL)
			return NULL;
	...
	case ASPFUNC_STAT:
		...
		if (asp->asp_slen > 0) {
			...
			memcpy(asp->data, asp->asp_status,
			       MIN(asp->asp_slen, i * ASP_CMDSIZ));
	...
	default:
		LOG(log_info, logtype_default, "ASPUnknown %d",
		    asp->cmdbuf[0]);
		break;
	}
