...
static int get_and_map_acl(char *name, char *rbuf, size_t *rbuflen)
{
	...
	*rbuf = 0;
	...
	if (S_ISDIR(st.st_mode)) {
		...
		EC_NEG1(mapped_aces =
			map_acl(POSIX_DEFAULT_2_DARWIN | dirflag, defacl,
				(darwin_ace_t *) rbuf, 0));
	}
	...
	EC_NEG1(tmp = map_acl(POSIX_ACCESS_2_DARWIN | dirflag,
			      ...
			      (darwin_ace_t *) (rbuf +
						mapped_aces *
						sizeof(darwin_ace_t)), 0));
	mapped_aces += tmp;
#endif				/* HAVE_POSIX_ACLS */

	LOG(log_debug, logtype_afpd, "get_and_map_acl: mapped %d ACEs",
	    mapped_aces);

	*darwin_ace_count = htonl(mapped_aces);
	*rbuflen +=
	    sizeof(darwin_acl_header_t) +
	    (mapped_aces * sizeof(darwin_ace_t));
