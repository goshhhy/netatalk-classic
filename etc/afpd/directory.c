		  ...
		  struct dir *dir)
{
	...
	if (NULL == (dp = opendir("."))) {
		LOG(log_debug, logtype_afpd,
		    "caseenumerate: opendir failed: %s", dir->d_u_name);
		return -1;
	}
	...
	if ((size_t) -1 ==
	    convert_string(vol->v_volcharset, CH_UCS2, path->u_name, -1,
			   u2_path, sizeof(u2_path)))
		LOG(log_debug, logtype_afpd,
		    "caseenumerate: conversion failed for %s",
		    path->u_name);
	...
	for (de = readdir(dp); de != NULL; de = readdir(dp)) {
		...
		if ((size_t) -1 ==
		    convert_string(vol->v_volcharset, CH_UCS2, de->d_name,
				   -1, u2_dename, sizeof(u2_dename)))
			continue;
		...
		if (strcasecmp_w(u2_path, u2_dename) == 0) {
			...
			if (of_stat(vol, path) == 0) {
				LOG(log_debug, logtype_afpd,
				    "caseenumerate: using dir: %s, path: %s",
				    de->d_name, path->u_name);
	...
	while (bitmap != 0) {
		...
		case DIRPBIT_FINFO:
			if (isad) {
				/* Fixes #2802236 */
				u_int16_t fflags;
				memcpy(&fflags, finder_buf + FINDERINFO_FRFLAGOFF, sizeof(u_int16_t));
