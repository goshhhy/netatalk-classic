		      ...
		      char *rbuf _U_, size_t *rbuflen)
{
	...
	*rbuflen = 0;
	if (NULL == (ofork = of_find(ofrefnum))) {
		LOG(log_error, logtype_afpd,
		    "afp_setforkparams: of_find(%d) could not locate fork",
		    ofrefnum);
		return (AFPERR_PARAM);
	}
	...
	if (bitmap == (1 << FILPBIT_DFLEN)
	    || bitmap == (1 << FILPBIT_EXTDFLEN)) {
		st_size = ad_size(ofork->of_ad, eid);
		err = -2;
		if (st_size > size &&
		    ad_tmplock(ofork->of_ad, eid, ADLOCK_WR, size,
			       st_size - size, ofork->of_refnum) < 0)
			goto afp_setfork_err;

		err = ad_dtruncate(ofork->of_ad, size);
		if (st_size > size)
			ad_tmplock(ofork->of_ad, eid, ADLOCK_CLR, size,
				   st_size - size, ofork->of_refnum);
		if (err < 0)
			goto afp_setfork_err;
		...
		if (ad_flush(ofork->of_ad) < 0) {
			LOG(log_error, logtype_afpd,
			    "afp_setforkparams(%s): ad_flush: %s",
